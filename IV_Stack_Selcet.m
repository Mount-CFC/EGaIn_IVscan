...
%%%%   Parameters setting %%%%%
scan_voltge = 3;      %voltage scan range
area = 110;           %unit:um^2

min_cur = -8;      % Min limit of y axis in I-V plot
max_cur = 3;       % Max limit of y axis in I-V plot
min_logG = -8;     % Min limit of y axis in LogG-V plot
max_logG = 0;      % Max limit of y axis in LogG-V plot

%%%%%%% Selection Setting %%%%%%%%
%Average conductance should HIGHER than this value between -0.9~-0.8V
high_conductance = -555; 
...
%     data_samplingV=struc.data{1,4}; %第一行第4列，提取sampling voltage，防止仪器参数有误
...
% transform to current density, A/cm2
area = area * 1e-8; %um2 to cm2

% 单位：log (A/cm2)
for i = 1:length(ForwardTraceCurrent)
    ForwardTraceCurrent{i} = log10(ForwardTraceCurrent{i} ./ area);
end
for j = 1:length(ReverseTraceCurrent)
    ReverseTraceCurrent{j} = log10(ReverseTraceCurrent{j} ./ area);
end
for k = 1:length(ForwardCurrent_slct)
    ForwardCurrent_slct{k} = log10(ForwardCurrent_slct{k} ./ area);
end
for m = 1:length(ReverseCurrent_slct)
    ReverseCurrent_slct{m} = log10(ReverseCurrent_slct{m} ./ area);
end
...
%I-V Forward
figure(1)
hist_forward = plot_IV(ForwardTraceBias , ForwardTraceCurrent, -scan_voltge,scan_voltge,min_cur,max_cur,150,150);
title('Forward', 'Interpreter', 'tex','FontSize',15)
hold on
[XFittedForward,YFittedForward] = master_curve(hist_forward,-scan_voltge,scan_voltge,min_cur,max_cur,150,150);%值与上面函数输入一致
plot(XFittedForward,YFittedForward,'-b','linewidth',1);

%I-V Reverse
figure(2)
hist_reverse = plot_IV(ReverseTraceBias, ReverseTraceCurrent, -scan_voltge,scan_voltge,min_cur,max_cur,150,150);
title('Reverse', 'Interpreter', 'tex','FontSize',15)
hold on
[XFittedReverse,YFittedReverse] = master_curve(hist_reverse,-scan_voltge,scan_voltge,min_cur,max_cur,150,150);%值与上面函数输入一致
plot(XFittedReverse,YFittedReverse,'-b','linewidth',1);

%I-V f+r
figure(3)
hist_f_r = plot_IV([ForwardTraceBias ReverseTraceBias], [ForwardTraceCurrent,ReverseTraceCurrent], -scan_voltge,scan_voltge,min_cur,max_cur,150,150);
...
plot(XFittedForward,YFittedForward,'-b','linewidth',2);
hold on
plot(XFittedReverse,YFittedReverse,'-k','linewidth',2);

%LogG-V f+r
figure(4)
hist_logG = plot_IV([ForwardTraceBias ReverseTraceBias], [ForwardTraceLogG,ReverseTraceLogG], -scan_voltge,scan_voltge,min_logG,max_logG,150,150);
title('logG - F&R', 'Interpreter', 'tex','FontSize',15)
ylabel('Conductance / LogG', 'Interpreter', 'tex','FontSize',15)
hold on
[XFittedLogG,YFittedLogG] = master_curve(hist_logG,-scan_voltge,scan_voltge,min_logG,max_logG,150,150);
...
% 筛选后的曲线的热力图：正扫+反扫的子图
figure(6)
set(gcf, 'unit','centimeters','Position', [5,10,35,12])
%Forward
subplot(131)
...
figure(31);
set(gcf, 'unit','centimeters','Position', [20,5,15,20])
subplot(211)
% plot(X_TVS(1:sp),Y_TVS(1:sp),'-k','linewidth',1.3);
scatter(X_TVS(1:sp),Y_TVS(1:sp),'filled','black','s');
ylabel({'ln(|\itJ|\rm/V^2)'},'Interpreter','tex','FontSize',15);
