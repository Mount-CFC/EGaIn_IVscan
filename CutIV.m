...
%     电流单位：A
    ForwardTraceCurrent{m} = abs(currentNew(ForwardIndex(ForwardTraceIndex(m)+1) : ForwardIndex(ForwardTraceIndex(m+1)))) .* 1e-3;
...
%     ReverseTraceCurrent{n} = log10(abs(currentNew(ReverseIndex(ReverseTraceIndex(n)+1) : ReverseIndex(ReverseTraceIndex(n+1)))) .* 1e6);
    ReverseTraceCurrent{n} = abs(currentNew(ReverseIndex(ReverseTraceIndex(n)+1) : ReverseIndex(ReverseTraceIndex(n+1)))) .* 1e-3;
